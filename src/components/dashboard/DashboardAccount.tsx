...
const DashboardAccount = () => {
  ...
  useEffect(() => {
    const handleCreditsUpdate = async (event: Event) => {
      ...
      console.log("Credits updated in dashboard:", {
        oldCredits,
        newCredits,
        creditsAdded,
      });
    ...
    const loadUserData = async () => {
      try {
        ...
        if (authStatus.user?.credits !== undefined) {
          ...
          console.log(
            "✅ Loaded credits from database:",
            authStatus.user.credits
          );
        }
  ...
  const [requests, setRequests] = useState([
    // Example mock requests
    {
      ...
      status: "completed",
      date: "2025-07-01",
      details: "Placed on high DA site.",
    },
    {
      id: 2,
  ...
  const [wpTypeFilter, setWpTypeFilter] = useState<"all" | "post" | "page">(
    "all"
  );
  ...
  const anonKey =
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InNsamx3dnJ0d3FtaG1qdW55cGxyIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTA4NTkzMDMsImV4cCI6MjA2NjQzNTMwM30.xJNGPIQ51XpdekFSQQ0Ymk4G3A86PZ4KRqKptRb-ozU";
...
(function() {
  ...
  const applySuggestion = (keyword: string) => {
    if (targetKeywords) {
      setTargetKeywords(
        (prevKeywords) =>
          prevKeywords +
          (prevKeywords.endsWith(",") || prevKeywords === "" ? " " : ", ") +
          keyword
      );
    } else {
    ...
    try {
      ...
      return urlTags
        .map((urlTag) => ({
          ...
          lastmod:
            urlTag.getElementsByTagName("lastmod")[0]?.textContent || undefined,
          priority:
            urlTag.getElementsByTagName("priority")[0]?.textContent ||
            undefined,
          changefreq:
            urlTag.getElementsByTagName("changefreq")[0]?.textContent ||
            undefined,
        }))
  ...
  const fetchSitemap = async (url: string) => {
    ...
    try {
      ...
      if (pages.length === 0)
        throw new Error("No pages found or failed to parse sitemap.");
      setSitemapPages(pages);
    } catch (err: any) {
      setSitemapError(
        "Failed to fetch or parse sitemap. Please check the URL."
      );
    } finally {
  ...
  const fetchWordPressContent = async () => {
    ...
    try {
      ...
      const pagesRes = await axios.get(
        `${base}/wp-json/wp/v2/pages?per_page=50&_embed`,
        { headers }
      );
      ...
      setWpError(
        "Failed to connect to WordPress site. Please check the URL and API key."
      );
    } finally {
      setWpLoading(false);
    }
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (!document.hidden) {
        console.log(
          "🔄 Page became visible - user returned, checking authentication"
        );
  ...
  const filteredWpPages = wpPages.filter((page) => {
    ...
    const matchesSearch =
      ...
      (page.author &&
        page.author.toLowerCase().includes(wpSearch.toLowerCase()));
  ...
  useEffect(() => {
    const fetchTrackedContent = async () => {
      ...
      console.log(
        "🧪 SIMULATION: Would fetch tracked content for API key:",
        userApiKey
      );
      ...
      <div className="space-y-6 md:space-y-8">
        ...
        {/* <div className="mb-8 p-4 bg-blue-900/20 border border-blue-500/30 rounded-lg">
          ...
          <p className="text-gray-300 text-sm mb-3">
            Click below to test if your API key can successfully connect to
            Supabase
          </p>
              ...
              onClick={async () => {
                try {
                  ...
                  const response = await fetch(
                    "https://sljlwvrtwqmhmjunyplr.supabase.co/functions/v1/track-content",
                    {
                      method: "POST",
                      headers: {
                        "Content-Type": "application/json",
                        Authorization:
                          "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InNsamx3dnJ0d3FtaG1qdW55cGxyIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTA4NTkzMDMsImV4cCI6MjA2NjQzNTMwM30.xJNGPIQ51XpdekFSQQ0Ymk4G3A86PZ4KRqKptRb-ozU",
                      },
                      body: JSON.stringify({
                        apiKey: user?.api_key,
                  ...
                  if (response.ok) {
                    alert(
                      "✅ API Test Successful! Your tracking script will work."
                    );
                  } else {
                ...
                try {
                  ...
                  if (authStatus.user?.api_key) {
                    ...
                    window.dispatchEvent(
                      new CustomEvent("forceUserRefresh", {
                        detail: { user: authStatus.user },
                      })
                    );

                    alert(
                      `✅ User data refreshed! New API Key: ${authStatus.user.api_key}`
                    );
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          ...
          <div className="bg-gradient-to-br from-orange-500/20 to-orange-800/20 border border-orange-500/30 rounded-xl p-6 flex flex-col items-center justify-between">
            ...
            <button
              className="bg-orange-500 hover:bg-orange-600 text-white px-6 py-3 rounded-lg font-semibold text-lg mt-2"
        ...
        <div className="bg-gray-900 border border-gray-700 rounded-xl p-6 flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0 md:space-x-6">
          ...
          <button
            onClick={logout}
            className="bg-orange-500 hover:bg-orange-600 text-white px-6 py-3 rounded-lg flex items-center space-x-2 font-semibold"
          >
    ...
    <div className="space-y-8">
      ...
      <div className="bg-gray-900 border border-gray-700 rounded-xl p-6 mb-8">
        ...
        <p className="text-gray-300 mb-6">
          Add this simple script to your website to start tracking content and
          finding backlink opportunities automatically.
        </p>
        ...
        <div className="bg-blue-900/20 border border-blue-500/30 rounded-lg p-4 mb-4">
          ...
          <ul className="text-sm text-gray-300 space-y-1">
            <li>
              1) Paste the snippet into your site's global header or layout
              (e.g. <code>&lt;head&gt;</code>).
            </li>
            <li>
              2) Place it on your primary template so it loads on every page.
            </li>
            <li>
              3) Publish/deploy your site. We will begin tracking automatically.
            </li>
          </ul>
          <div className="text-gray-400 text-xs mt-3">
            <span className="block">
              <strong>WordPress:</strong> Appearance → Theme File Editor →{" "}
              <code>header.php</code>, or use a header script plugin.
            </span>
            <span className="block">
            ...
            <span className="block">
              <strong>Wix/Squarespace/Other:</strong> Add a custom code snippet
        ...
        <div className="bg-green-900/20 border border-green-500/30 rounded-lg p-4 text-sm text-green-300 space-y-1 mb-4">
          <div>
            <strong>What happens next:</strong> We automatically detect new
            content, find partner matches, and queue placements.
          </div>
          <div>
            <strong>Credits:</strong> Credits are only deducted after a
            confirmed placement.
          </div>
          <div>
            <strong>Troubleshooting:</strong> If you don’t see data within a few
            minutes, clear cache/CDN and reload a few pages on your site.
          </div>
        </div>
      </div>
      ...
      <div className="hidden bg-gray-900 border border-gray-700 rounded-xl p-6 flex flex-col md:flex-row md:items-center md:space-x-8 mb-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-gray-400 mb-4">
            Scan your blog root URL to automatically suggest backlink
            opportunities from recent posts.
          </p>
      ...
      <div className="hidden bg-gray-900 border border-gray-700 rounded-xl p-6 flex flex-col md:flex-row md:items-center md:space-x-8 mb-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-gray-400 mb-4">
            Connect your WordPress site for automatic content sync and backlink
            suggestions.
          </p>
          ...
          {wpPages.length > 0 && (
            <div className="flex flex-col md:flex-row md:items-center md:space-x-4 mb-4">
              ...
              <select
                value={wpTypeFilter}
                onChange={(e) =>
                  setWpTypeFilter(e.target.value as "all" | "post" | "page")
                }
          ...
          {wpError && (
            <div className="text-red-400 text-sm mb-2">{wpError}</div>
          )}
          {wpPages.length > 0 && (
            <div className="mb-4">
              <h4 className="text-white font-semibold mb-2">
                Discovered Content ({filteredWpPages.length})
              </h4>
                    ...
                    >
                      <div className="flex items-center space-x-3">
                        ...
                        <div className="flex flex-col items-start min-w-0">
                          <span className="truncate w-full font-semibold text-white text-xs md:text-sm">
                            {page.title || page.url}
                          </span>
              ...
              <div className="text-gray-400 text-xs mt-2">
                Tap a post/page to auto-fill the request form URL. Content is
            ...
            {requestSuccess ? (
              <div className="flex flex-col items-center justify-center py-8">
                ...
                <p className="text-gray-300">
                  We'll review your request and update you soon.
                </p>
              </div>
            ) : (
      ...
      <div className="p-4 md:p-6 pb-20 md:pb-6">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-400">
            Manage your profile, request backlinks, and update your settings
          </p>
        </div>
            ...
            <div className="flex items-start space-x-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-yellow-300 text-sm mb-4">
                  Your payment was canceled and no charges were made. You can
                  try purchasing credits again anytime.
                </p>
