...
<body>
  ...
  async registerUser(email, password, website, niche) {
    try {
      ...
      // When a user already exists, Supabase returns success but with empty identities
      if (
        data.user &&
    ...
    try {
      ...
      const topKeywords = Object.entries(keywordMap)
        .map(([word, info]) => ({
          word,
          count: info.count,
          avgDensity: info.density.length
            ? info.density.reduce((a, b) => a + b, 0) / info.density.length
            : 0,
          urls: info.urls,
        }))
  ...
  async updateUserProfile(website, niche) {
    try {
      ...
      if (error.message?.includes("schema cache")) {
        errorMessage =
          "Database schema error. Please try again or contact support.";
      } else if (error.message?.includes("unauthorized")) {
        errorMessage =
          "Not authorized to update profile. Please sign in again.";
  ...
  async updateUserCredits(userId, creditsToAdd, paymentDetails) {
    try {
      ...
      try {
        ...
        console.log("📊 Billing history result:", {
          billingData,
          billingError,
        });
