...
function generateAnchorTextSuggestions(
  keywords: string[],
  targetUrl: string
): string[] {
...
async function getNicheProximityScore(
  sourceNiche: string,
  targetNiche: string
): Promise<number> {
      ...
      try {
        ...
        if (scores.overall >= 0.1) {
          const anchorSuggestions = generateAnchorTextSuggestions(
            targetContent.keywords || [],
            sourceContent.url // error found - need to change sourceContent.users.website -> .url
          );
...
serve(async (req: Request) => {
  ...
  try {
    const { contentId, userId, forceReprocess }: MatchingRequest =
      await req.json();

    if (!contentId || !userId) {
      return new Response(
        JSON.stringify({ error: "Missing contentId or userId" }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        }
    ...
    if (opportunities.length > 0) {
      ...
      const autoApproved =
        insertedOpportunities?.filter((op) => op.auto_approved).length || 0;
