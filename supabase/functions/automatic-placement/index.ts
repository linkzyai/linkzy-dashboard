...
interface PlacementResult {
  ...
  placementMethod?: 'wordpress_api' | 'javascript_injection' | 'manual_fallback';
}
...
async function attemptJavaScriptPlacement(opportunity: any, targetDomainMetrics: any): Promise<PlacementResult> {
...
function findOptimalInsertionPoint(content: string, keywords: string[]): { position: number; context: string } {
...
function generateContextualSentence(anchorText: string, targetUrl: string, keywords: string[]): string {
  const templates = [
...
async function processSuccessfulPlacement(opportunityId: string, placementResult: PlacementResult) {
...
async function processFailedPlacement(opportunityId: string, error: string, responseTime?: number) {
  // Update opportunity status
  await supabase
    .from('placement_opportunities')
    .update({
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (opportunity.status !== 'approved' && !manualOverride) {
    ...
    if (platformInfo.isWordPress && platformInfo.hasRestAPI && targetDomainMetrics?.wordpress_api_enabled) {
      ...
      placementResult = await attemptJavaScriptPlacement(opportunity, targetDomainMetrics || { website: opportunity.target_user.website });
    }
